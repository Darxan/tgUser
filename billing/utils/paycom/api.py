...
from ..validator import UserController
...
class PaycomAPI:
    ...
    def check_perform_transaction(self):
        user = UserController(request_id=self.request.id)
        if user.validate(account_params=self.request.params):
            """ 
                Comment: User successfully validated 
            """
            data = {
                "result": user.response_message
            }
            self.json_content = json.dumps(data)
        else:
            self.json_content = self.response.send(result=None, error=user.response_message)
        ...
        if transaction.exist():
            ...
            if transaction.check_transaction_state():
                if transaction.transaction_is_expired():
                    transaction.cancel_transaction(reason=Transactions.REASON_CANCELLED_BY_TIMEOUT)
                    """ 
            ...
            if user.validate(account_params=self.request.params) is False:
                ...
                if (now_in_milliseconds - self.request.params['time']) > Transactions.TIMEOUT:
    ...
    def perform_transaction(self):
        ...
        if transaction.exist():
            ...
            elif transaction.check_transaction_state(state=Transactions.STATE_COMPLETED):
                """
            ...
            self.json_content = self.response.error(code=Transactions.TRANSACTION_NOT_FOUND,
        ...
        if transaction.exist():
            if transaction.check_transaction_state(state=Transactions.STATE_CREATED):
                ...
                if transaction.transaction.state == 2:
                    self.json_content = self.response.error(code=Transactions.TRANSACTION_CAN_NOT_CANCEL,
    ...
    def get_statement(self):
        all_transactions = Transactions.get_statement(_from=self.request.params['from'], _to=self.request.params['to'])
        self.json_content = all_transactions

    def change_password(self):
        try:
            if self.request.params['password'] != os.environ['PAYCOM_PROTOCOL_APP_TESTKEY']:
                os.environ['PAYCOM_PROTOCOL_APP_TESTKEY'] = self.request.params['password']
                data = {
                    "id": self.request.id,
                    "result": {
                        "success": True
